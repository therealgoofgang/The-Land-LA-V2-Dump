...
RegisterNUICallback(Receive.click, function(option, cb)
    ...
    elseif type == "Seats" then
        if LocalPlayer.state.cuffed	then 
            lib.notify({ type = 'error', description = 'You are cuffed you can not do this!'})
            return 
        end

        if IsPedArmed(PlayerPedId(), 7) then
            lib.notify({ type = 'error', description = 'You can not do this while holding a weapon!'})
            return
        end
...
RegisterCommand("vehiclemenu", function()
    -- if LocalPlayer.state.cuffed	then return end
