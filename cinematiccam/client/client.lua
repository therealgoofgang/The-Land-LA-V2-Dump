...
Citizen.CreateThread(function()
    ...
    while true do
        -- Only run at high frequency when menu is open or camera is active
        if (_menuPool:IsAnyMenuOpen() or cam) then
            Citizen.Wait(1)
        else
            Citizen.Wait(500) -- Maximum idle efficiency - script does nothing when not in use
        end
...
if ( not Config.noMetaGaming ) then
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if not (camMenu and camMenu:Visible() and cam) then
                Citizen.Wait(5000) -- Sleep for 5 seconds when completely inactive
            else
                Citizen.Wait(500)
...
function GetForwardVectorFromRotation(rot)
...
local cameraEnt = false
local cemeraEntModel = 'ch_prop_ch_camera_01'
...
function ProcessCamControls()
    ...
    local correctedRot = vector3(camRot.x, camRot.y, (camRot.z + 180.0) % 360.0)
    local forward = GetForwardVectorFromRotation(camRot)
    local backOffset = 0.5
    local behindCamCoords = camCoords - forward * backOffset

    if not DoesEntityExist(cameraEnt) then
        RequestModel(cemeraEntModel)
        while not HasModelLoaded(cemeraEntModel) do
            Citizen.Wait(10) -- Changed from 0 to 10ms
        end

        cameraEnt = CreateObject(GetHashKey(cemeraEntModel), behindCamCoords.x, behindCamCoords.y, behindCamCoords.z, true, true, false)
        SetEntityRotation(cameraEnt, correctedRot.x, correctedRot.y, correctedRot.z, 2, true, false)
        SetEntityHeading(cameraEnt, correctedRot.z)
        SetEntityCollision(cameraEnt, false, false)
        FreezeEntityPosition(cameraEnt, true)
    else
        SetEntityCoords(cameraEnt, behindCamCoords.x, behindCamCoords.y, behindCamCoords.z)
        SetEntityRotation(cameraEnt, correctedRot.x, correctedRot.y, correctedRot.z, 2, true, false)
        SetEntityHeading(cameraEnt, correctedRot.z)
        SetEntityCollision(cameraEnt, false, false)
        FreezeEntityPosition(cameraEnt, true)
...
function ToggleCam(flag, fov)
    if (flag) then
        ...
        EndFreeCam()
        if (DoesEntityExist(cameraEnt)) then
            DeleteEntity(cameraEnt)
            cameraEnt = false
        end
        _menuPool:RefreshIndex()
    end
...
-- REMOVED: Catastrophic Wait(0) filter loop that ran every frame
-- Filters are already applied in ApplyFilter() and ChangeFilterIntensity()
-- No need to continuously reapply them 60 times per second
