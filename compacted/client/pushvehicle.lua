...
if Config.vehilepush.TextUI then
    if not Config.vehilepush.target then
        uiOpen = false
            ...
            while uiThreadRunning do
                ...
                if pushing then
                    ...
                    if not vehicle then
                        ...
                        if Config.vehilepush.blacklist[model] then
                            if uiOpen then
                                lib.hideTextUI()
                                uiOpen = false
                            end
                        else
                            local health = GetVehicleEngineHealth(vehicle) <= Config.vehilepush.healthMin and true or false
                            if health then
...
keybind = lib.addKeybind({
    ...
    defaultKey = Config.vehilepush.PushKey,
    onPressed = function(self)
        if Config.vehilepush.target then return end
...
lib.addKeybind({
    ...
    defaultKey = Config.vehilepush.TurnLeftKey,
    onPressed = function(self)
...
lib.addKeybind({
    ...
    defaultKey = Config.vehilepush.TurnRightKey,
    onPressed = function(self)
        if not pushing then return end
	if LocalPlayer.state.intrunk then return end
...
if Config.vehilepush.target then
	local options = {
		{
			name = 'startPushing',
		...
		if LocalPlayer.state.intrunk then return false end
                ...
                local health = GetVehicleEngineHealth(entity) <= Config.vehilepush.healthMin and true or false
	...
	local targetSystem = string.lower(Config.vehilepush.targetSystem)
	if targetSystem == 'qtarget' then
		if Config.vehilepush.Usebones then
