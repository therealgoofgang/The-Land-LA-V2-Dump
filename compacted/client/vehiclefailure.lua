...
lib.callback.register('isVehicleRepairAllowed', function()
	local currentVehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)

	local bulletProofTable = exports.crn_ui_svdden:fetchBulletProofModels()
    for i = 1, #bulletProofTable do
		if bulletProofTable[i] == GetEntityModel(currentVehicle) then return false end
	end
...
RegisterNetEvent('iens:repair', function()
	if lib.progressBar({
		...
		if isPedDrivingAVehicle() then
			local ped = GetPlayerPed(-1)
			vehicleR = GetVehiclePedIsIn(ped, false)
			if IsNearMechanic() then
				notification("Wait while the mechanic repair your car")
				FreezeEntityPosition(vehicleR, true)
				Wait(10000)
				SetVehicleUndriveable(vehicleR,false)
				SetVehicleFixed(vehicleR)
				healthBodyLast=1000.0
				healthEngineLast=1000.0
				healthPetrolTankLast=1000.0
				SetVehicleEngineOn(vehicleR, true, false )
				notification("The mechanic repaired your car!")
				FreezeEntityPosition(vehicleR, false)
				return
			end
...
local frameThreadAlive = false
local function startFrameThread()
	if frameThreadAlive then return end
	if not (cfg.torqueMultiplierEnabled or cfg.preventVehicleFlip or cfg.limpMode) then return end
	frameThreadAlive = true
	Citizen.CreateThread(function()
		while cache.vehicle or pedInSameVehicleLast do
			Citizen.Wait(0)
		...
		frameThreadAlive = false
	end)
end

local damageThreadAlive = false
local function startDamageThread()
	if damageThreadAlive then return end
	damageThreadAlive = true
	Citizen.CreateThread(function()
		while cache.vehicle or pedInSameVehicleLast do
			Citizen.Wait(50)
		local ped = GetPlayerPed(-1)
		...
		damageThreadAlive = false
	end)
end

-- Start threads when entering a vehicle. The cache callback fires AFTER
-- cache.vehicle has been updated, so cache.vehicle is truthy here.
...
-- Handle resource start while already in a vehicle (e.g. /restart). cache.vehicle
-- is populated by ox_lib before user resources execute, so this is safe.
