...
local function setPedHeadOverlays(ped, headOverlays)
	if not headOverlays then
		return log.errorf("[game] [util] [setPedHeadOverlays] param `headOverlays` is nil.")
	end
...
local function setPedComponent(ped, component)
	if component then
		...
		local maxDrawable = GetNumberOfPedDrawableVariations(ped, component.component_id) - 1
		if component.drawable > maxDrawable then
			component.drawable = math.max(0, maxDrawable)
			clampedAppearance = true
		end

		local maxTexture = GetNumberOfPedTextureVariations(ped, component.component_id, component.drawable) - 1
		if component.texture > maxTexture then
			component.texture = math.max(0, maxTexture)
			clampedAppearance = true
		end
...
local function setPedProp(ped, prop)
	if prop then
		local maxDrawable = GetNumberOfPedPropDrawableVariations(ped, prop.prop_id) - 1
		if prop.drawable > maxDrawable then
			prop.drawable = maxDrawable
			clampedAppearance = true
		end
		...
		if prop.drawable < 0 then
			...
			local maxTexture = GetNumberOfPedPropTextureVariations(ped, prop.prop_id, prop.drawable) - 1
			if prop.texture > maxTexture then
				prop.texture = math.max(0, maxTexture)
				clampedAppearance = true
			end
...
local function setPlayerAppearance(appearance)
	if appearance then
		clampedAppearance = false
		setPlayerModel(appearance.model)
		setPedAppearance(cache.ped, appearance)

		if clampedAppearance then
			lib.notify({
				title = "Appearance",
				description = "Some appearance values were adjusted to fit available options",
				type = "inform",
